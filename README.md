...
- CBS MSFE：核查型面试，在筛简历时淘汰大多数人，面试通过率约为70%

此处重点介绍 MIT MFin 和 CBS MSFE 的面试准备 tips.

A. MIT MFin, 部分内容 credit to Eddie (MIT MFin)：
1. 记得在面试前准备好ID，最好是护照，带英文名的港澳通行证也可以，身份证没有英文名不满足要求
2. 面试最重要的是展现你的energy，你需要足够自信
3. 注意房间的回音、收音、光线问题，本人就直接被面试官指出耳机有电流声（麻
4. 开场时面试官会习惯性的开一些small talk，不要过于紧张，让自己看起来自然一点
5. 对方大概率并不理解你简历里面的terminology，学会把专业性的东西用简单的语言表达
6. 描述一个project时，内容具体外，使用总分总的逻辑表达，并和soft skills挂钩
7. 注意面试的flow，适当的增添对话引导。能够逗笑面试官是一件很加分的事情

B. CBS MSFE（由CBS MSFE Class of ‘27 4位校友提供）：
1. 项目录取比较多元（目标量化/非量化/读博都有，25 fall 录取 CUHKsz 一位读博、一位非量化和两位量化的校友），非量化同学也完全可以target。面试不会问的很量化，更多还是看对于 CBS 和 general finance 的兴趣
2. 建议提前关注面试官LinkedIn，根据她的经历反推面试问题。比如被分配到做推荐系统的面试官就会拷问机器学习、深度学习和最优化的知识点
